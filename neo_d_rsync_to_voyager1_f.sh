...
echo ---------------- "Rsync from Neo_D: ${fromNeoD} to Voyager1_F: $toVoyager1F" ---------------- >> logs/rsync.log
date >> logs/rsync.log
echo
...
rsync -azhu --log-file='logs/rsync.log' --delete-after --ipv6 --progress  --exclude='$RECYCLE.BIN/' --exclude='/System Volume Information/' --exclude='DumpStack.log.tmp' --exclude='desktop.ini' --exclude='ext_hd' --exclude='files_bkp'  "$fromNeoD"  "$toVoyager1F"
...
ls -lh $toVoyager1F >> logs/rsync.log
echo ----------------"Backup finished"---------------- >> logs/rsync.log
echo